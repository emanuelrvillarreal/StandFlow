    ...
    <div className="min-h-screen bg-ink-950">
      <nav className="bg-ink-900/90 backdrop-blur border-b border-ink-700 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-ink-800 border border-accent/30 rounded-lg flex items-center justify-center">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {events.map(ev => currentUser ? (
            <div key={ev.id}
              className="group relative bg-ink-800 rounded-2xl border border-ink-600 overflow-hidden hover:border-accent/50 transition-all">
              {ev.posterImage ? (
                <div className="relative aspect-[3/2] overflow-hidden">
                  <img src={ev.posterImage} alt={ev.name} className="w-full h-full object-cover" />
            ...
            <button key={ev.id}
              onClick={() => navigate(`/events/${ev.id}/map`)}
              className="group text-left bg-ink-800 rounded-2xl border border-ink-600 overflow-hidden hover:border-accent/50 transition-all">
              {ev.posterImage ? (
                <div className="relative">
                  <img src={ev.posterImage} alt={ev.name} className="w-full h-auto block" />
                  <span className={`absolute top-3 right-3 text-xs font-semibold px-2.5 py-1 rounded-full ${STATUS_STYLES_ON_IMAGE[ev.status]}`}>
                    {STATUS_LABELS[ev.status]}
                  </span>
                </div>
